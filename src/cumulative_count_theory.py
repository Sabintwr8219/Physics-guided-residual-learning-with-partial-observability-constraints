...
def build_ad_times() -> pd.DataFrame:
    """
    Compute upstream-arrival and stopbar-departure times for all vehicles.
    ...
    require_columns(df, required, "traj_nb_filtered.csv")

    has_vehid = "vehID" in df.columns
    ...
    df["s_rel_stop_m"] = pd.to_numeric(df["s_rel_stop_m"], errors="coerce")

    if has_vehid:
        df["vehID"] = pd.to_numeric(df["vehID"], errors="coerce")
    ...
    for run_id in RUN_IDS:
        run_df = df[df["run_id"] == int(run_id)].copy()

        if run_df.empty:
            print(f"[Run {run_id:03d}] No filtered trajectory rows found. Skipping.")
            continue

        for veh_uid, group in run_df.groupby("veh_uid", sort=False):
            arrival_time = first_cross_time(group, Y_UPSTREAM_DETECTOR_M)
            departure_time = first_cross_time(group, Y_STOPBAR_M)

            record = {
                "run_id": int(run_id),
                "veh_uid": str(veh_uid),
                "t_arr_updet_sec": arrival_time,
                "t_dep_stopbar_sec": departure_time,
            }

            if has_vehid:
                veh_ids = group["vehID"].dropna()
                record["vehID"] = int(veh_ids.iloc[0]) if len(veh_ids) else np.nan

            rows.append(record)

        print(f"[Run {run_id:03d}] Vehicles processed: {run_df['veh_uid'].nunique():,}")
